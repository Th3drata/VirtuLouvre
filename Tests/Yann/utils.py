...
def draw_velocity(player, font):
    
    velocity = player.velocity
    text = f"Velocity: {velocity[0]:.2f}, {velocity[1]:.2f}, {velocity[2]:.2f}"
    texture_id, width, height = render_text_to_texture(text, font)
    ...
    glColor3f(1, 1, 1)
    glBegin(GL_QUADS)
    glTexCoord2f(0, 0); glVertex2f(10, 470)
    glTexCoord2f(1, 0); glVertex2f(10 + width, 470)
    glTexCoord2f(1, 1); glVertex2f(10 + width, 470 + height)
    glTexCoord2f(0, 1); glVertex2f(10, 470 + height)
    glEnd()
    glDisable(GL_TEXTURE_2D)
