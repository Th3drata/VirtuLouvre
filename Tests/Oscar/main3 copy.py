...
def display_settings(screen, font, back_button_image, current_tab, settings_background):
    ...
    if current_tab == "Vidéo":
        ...
        # Couleur du bouton
        reset_button_color = (200, 50, 50)  # Rouge
        reset_button_hover_color = (150, 30, 30)  # Rouge foncé
        ...
        reset_text = font.render("Réinitialiser les touches", True, (255, 255, 255))
...
def start_game(screen, font, background_image, dimensions_possibles, current_state):
    global master_volume, video_capture
    pygame.init()  # Initialiser Pygame
    clock = pygame.time.Clock()
    ...
    while True:
        ...
        if current_state == "game_menu":
            ...
            # Afficher les paramètres avec les onglets
            (
                back_rect,
                tab_buttons,
                left_rect,
            ...
            # Gérer les événements des paramètres
            if event.type == pygame.MOUSEBUTTONDOWN:
                if back_rect.collidepoint(event.pos):
                    current_state = "game_menu"
                for tab_button in tab_buttons:
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    ...
                    if current_tab == "Vidéo":
                        ...
                        if reset_button_rect.collidepoint(event.pos):
                            global controls
                            controls = DEFAULT_CONTROLS.copy()
                            save_controls()  # Sauvegarder les contrôles par défaut
                    ...
                    controls[waiting_for_key] = (key_name, controls[waiting_for_key][1])
                    waiting_for_key = None
                    # Sauvegarder les contrôles après chaque modification
                    save_controls()
                elif (
                    ...
                    if current_tab == "Audio" and volume_slider_rect.collidepoint(
                        ...
                        # Mettre à jour le volume de tous les sons
                        walk_sound_effect.set_volume(master_volume)
                        # Sauvegarder les contrôles après chaque modification du volume
                        save_controls()
...
def save_controls():
    ...
    config_data = {"controls": controls, "master_volume": master_volume}
