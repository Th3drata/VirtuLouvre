...
def draw_skybox(texture_id, radius=1000, quality=64):
    ...
    gluSphere(
        gluNewQuadric(), radius, quality, quality
    )  # Dessiner la sphère (rayon, détails)
    glPopMatrix()
...
def display_settings(screen, font, back_button_image, settings_background, current_tab):
    ...
    tab_height,
    tab,
    (0, 0, 0),  # Couleur du texte (ex: noir)
    font,
    (200, 200, 200),  # Couleur normale
    (150, 150, 150)   # Couleur hover
))
...
def start_game(screen, font, background_image, dimensions_possibles, current_state):
    ...
    menu_screen = pygame.Surface(
        (display[0], display[1]), pygame.SRCALPHA
    ...
    # Préparer les vertices pour la collision
    collision_vertices = []
    for face in faces:
        for vertex_idx in face[0]:
            collision_vertices.append(np.array(vertices[vertex_idx]))

    # Créer le joueur avec les vertices pour la collision
    player = Player(
        45, (display[0] / display[1]), 0.1, 200, vertices=collision_vertices
    )
    ...
    # Activer le VSync pour éviter le tearing
    pygame.display.gl_set_attribute(pygame.GL_SWAP_CONTROL, 1)

    # Optimiser les états OpenGL
    glEnable(GL_DEPTH_TEST)
    glEnable(GL_CULL_FACE)
    glCullFace(GL_BACK)

    # Réduire la qualité du skybox pour de meilleures performances
    sky_quality = 32  # Réduire de 64 à 32
    ...
    while True:
        ...
        if current_state == "game":
            # Désactiver les états OpenGL non nécessaires
            glDisable(GL_BLEND)
            glDisable(GL_TEXTURE_2D)

            # Rendu optimisé
            player.apply()
            draw_skybox(sky_texture, radius=1000, quality=sky_quality)
            draw_textured_floor(floor_texture)

            # Activer les textures uniquement pour le modèle
            glEnable(GL_TEXTURE_2D)
            draw_model(model_vbo, model_vertex_count)
            glDisable(GL_TEXTURE_2D)

            # Afficher FPS uniquement toutes les 0.5 secondes
...
def main_menu():
    ...
    buttons = [
        Button(screen_width // 2 - button_width // 2, start_y, button_width, button_height, "Jouer", WHITE, font, blue, hover_blue),
        Button(screen_width // 2 - button_width // 2, start_y + button_height + button_spacing, button_width, button_height, "Paramètres", WHITE, font, blue, hover_blue),
        Button(screen_width // 2 - button_width // 2, start_y + 2 * (button_height + button_spacing), button_width, button_height, "Crédits", WHITE, font, blue, hover_blue),
        Button(screen_width // 2 - button_width // 2, start_y + 3 * (button_height + button_spacing), button_width, button_height, "Quitter", WHITE, font, blue, hover_blue)
    ]
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                for i, button in enumerate(buttons):
                    button.rect.topleft = (screen_width // 2 - button_width // 2, start_y + i * (button_height + button_spacing))
        ...
        if current_state == "parametres":
            ...
            start_game(screen, font, dimensions_possibles, dimensions_possibles, current_state)
            pygame.display.flip()
