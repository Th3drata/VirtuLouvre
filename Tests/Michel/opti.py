        ...
        with open(filename, 'r') as file:
            for line in file:
                if line.startswith('newmtl '):
                    ...
                    if current_material:
                        color = [float(c) for c in line.strip().split()[1:]]
...
def main():
    ...
    model_path = os.path.join(current_dir, 'cuba.obj')
    vertices, faces = load_obj(model_path)
    mtl_path = os.path.join(current_dir, 'cuba.mtl')
    ...
    while True:
        ...
        grid()
        draw_model_vbo(model_vbo, model_vertex_count)
        get_pos(player, font)
