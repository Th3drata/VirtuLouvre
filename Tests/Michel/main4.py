...
class Player:
    ...
    def update(self, delta_time):
        new_position = self.position + self.velocity + delta_time # Calculer la nouvelle position
...
def draw_minimap():
    minimap_screen = pygame.Surface((200, 200), pygame.SRCALPHA)  # Créer une surface pour la minimap
    minimap_screen.fill((0, 0, 0, 0))  # Fond transparent
    pygame.draw.circle(minimap_screen, WHITE, (100, 100), 50, 0)
...
def start_game(screen, font, background_image, dimensions_possibles, current_state):
    ...
    pygame.display.set_caption(
        "3D View - Press ESC to toggle menu"
    )  # Titre de la fenêtre
    ...
    )  # Créer une surface pour le menu avec un canal alpha  # Initialiser le menu à fermé
    ...
    while True:
        delta_time = clock.tick(165) / 1000.0  # Calculer le temps écoulé
        ...
        for event in pygame.event.get():  # Pour chaque événement Pygame
            ...
            if event.type == pygame.KEYDOWN and event.key == pygame.K_ESCAPE:
                current_state = "game_menu" if current_state != "game_menu" else "game"# Toggle menu
                pygame.event.set_grab(current_state != "game_menu")
                pygame.mouse.set_visible(current_state == "game_menu")
            ...
            if current_state == "bonjour":  # Si le menu est ouvert
                if (
        ...
        if current_state == "game_menu":
            ...
            # Define buttons
            buttons1 = [
                Button(screen.get_width() // 2 - 100, 300, 200, 50, "Resume", WHITE, font, BLUE, HOVER_BLUE),
                Button(screen.get_width() // 2 - 100, 400, 200, 50, "Settings", WHITE, font, BLUE, HOVER_BLUE),
                Button(screen.get_width() // 2 - 100, 500, 200, 50, "Quit", WHITE, font, BLUE, HOVER_BLUE),
            ]
            ...
            for button in buttons1:
                ...
                # Check for clicks without polling for events again
                if pygame.mouse.get_pressed()[0]:  # Left mouse button
                    if button.rect.collidepoint(mouse_pos):
                        if button.text == "Resume":
                            current_state = "game"  # Return to the game
                            pygame.event.set_grab(True)
                            pygame.mouse.set_visible(False)
                        elif button.text == "Settings":
                            current_state = "bonjour" if current_state != "bonjour" else "game_menu" # Show the FOV slider
            ...
            pygame.draw.rect(menu_screen, (20, 20, 20, 180), 
                        (menu_x, menu_y, menu_width, menu_height),
                        border_radius=15)
            # Title for the FOV settings
            title_text = "FOV Camera Settings"
            title = font.render(title_text, True, (255, 255, 255))  # White text
            title_rect = title.get_rect(center=(screen.get_width() // 2, 150))
            menu_screen.blit(title, title_rect)

            # Slider bar
            bar_x, bar_y = screen.get_width() // 2 - 210, 300  # Position of the slider
            bar_width, bar_height = 420, 20  # Dimensions of the slider

            # Draw the slider background
            pygame.draw.rect(menu_screen, (100, 100, 100), (bar_x, bar_y, bar_width, bar_height), border_radius=10)
            pygame.draw.rect(menu_screen, (200, 200, 200), (bar_x, bar_y, bar_width, bar_height), 2, border_radius=10)

            # Calculate the position of the slider handle
            handle_x = int(bar_x + (slider_value - 70) / 80 * bar_width)
            handle_y = bar_y + bar_height // 2

            # Draw the slider handle
            pygame.draw.circle(menu_screen, (0, 200, 0), (handle_x, handle_y), 10)

            # Display the current FOV value
            value_text = f"FOV: {int(slider_value)}°"
            value_surface = font.render(value_text, True, (255, 255, 255))
            value_rect = value_surface.get_rect(center=(screen.get_width() // 2, 250))
            menu_screen.blit(value_surface, value_rect)
